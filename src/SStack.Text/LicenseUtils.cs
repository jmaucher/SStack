...
// License: https://raw.github.com/SStack/SStack/master/license.txt
...
namespace SStack
{
    ...
    {
        None = 0,
        All = Premium | Text | Client | Common | Redis | OrmLite | SStack | Server | Razor | Admin | Aws,
        RedisSku = Redis | Text,
    ...
    public enum QuotaType
    {
        Operations,      //SStack
        Types,           //Text, Redis
        Fields,          //SStack, Text, Redis, OrmLite
    ...
    /// <summary>
    /// Public Code API to register commercial license for SStack.
    /// </summary>
    ...
    public static class LicenseUtils
    {
        ...
        private const string ContactDetails = " Please see SStack.net or contact team@SStack.net for more details.";
        ...
        {
            private const string UpgradeInstructions = " Please see https://SStack.net to upgrade to a commercial license or visit https://github.com/SStackV3/SStackV3 to revert back to the free SStack v3.";
            internal const string ExceededTextTypes = "The free-quota limit on '{0} SStack.Text Types' has been reached." + UpgradeInstructions;
            ...
            internal const string ExceededSStackOperations = "The free-quota limit on '{0} SStack Operations' has been reached." + UpgradeInstructions;
        ...
        {
            public const int SStackOperations = 10;
        ...
        public static void AssertEvaluationLicense()
        {
            if (DateTime.UtcNow > new DateTime(2013, 12, 31))
                throw new LicenseException("The evaluation license for this software has expired. " +
                    "See https://SStack.net to upgrade to a valid license.").Trace();
        }
        ...
        public static void AssertValidUsage(LicenseFeature feature, QuotaType quotaType, int count)
        {
            ...
            switch (feature)
            {
                ...
                case LicenseFeature.SStack:
                    switch (quotaType)
                    {
                        case QuotaType.Operations:
                            ApprovedUsage(licensedFeatures, feature, FreeQuotas.SStackOperations, count, ErrorMessages.ExceededSStackOperations);
                            return;
                    }
                    break;
        ...
        static class _approved
        {
            ...
            {
                "SStack.ServiceClientBase+AccessToken",
                "SStack.JsonHttpClient+AccessToken",
                "SStack.RabbitMq.RabbitMqProducer+AccessToken",
                "SStack.Messaging.RedisMessageQueueClient+AccessToken",
                "SStack.Messaging.RedisMessageProducer+AccessToken",
                "SStack.Aws.AwsClientUtils+AccessToken",
            };

            internal static readonly HashSet<string> __dlls = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            {
                "SStack.HttpClient.dll",
                "SStack.Client.dll",
                "SStack.RabbitMq.dll",
                "SStack.Aws.dll",
                "<Unknown>"
