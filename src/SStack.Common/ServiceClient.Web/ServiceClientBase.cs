...
using SStack.Net30.Collections.Concurrent;
...
namespace SStack.ServiceClient.Web
{
 ...
 : IServiceClient, IRestClient
...
#endif
    {
        ...
        /// Note that you should NOT consume the response stream as this is handled by SStack
        /// </summary>
        ...
        /// response to a custom ErrorResponse DTO type instead of SStack's ErrorResponse class. In case ex is a
            ...
            try
            {
                if (WebRequestUtils.ShouldAuthenticate(ex, this.UserName, this.Password))
                {
                    ...
                    if (webEx != null && webEx.Response != null)
                    {
                        ...
                        var doAuthHeader = headers[SStack.Common.Web.HttpHeaders.WwwAuthenticate];
                        ...
                        {
                            this.authInfo = new SStack.ServiceClient.Web.AuthenticationInfo(doAuthHeader);
