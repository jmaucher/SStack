...
namespace SStack.WebHost.Endpoints.Metadata
{
    ...
    public abstract class BaseMetadataHandler : HttpHandlerBase, ISStackHttpHandler
    {
            ...
            if (operationName != null)
            {
                ...
                if (op.Routes.Count > 0)
                {
                    ...
                    foreach (var route in op.Routes)
                    {
                        ...
                        if (!isSoap)
                        {
                            var path = "/" + PathUtils.CombinePaths(EndpointHost.Config.SStackHandlerFactoryPath, route.Path);
