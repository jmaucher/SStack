...
using System.Web;
using SStack.Common;
using SStack.MiniProfiler.UI;
using SStack.ServiceHost;
using SStack.Text;
using SStack.WebHost.Endpoints.Extensions;
using SStack.WebHost.Endpoints.Support;
using HttpRequestWrapper = SStack.WebHost.Endpoints.Extensions.HttpRequestWrapper;

namespace SStack.WebHost.Endpoints
{
    public class SStackHttpHandlerFactory
        : IHttpHandlerFactory
    {
        ...
        static SStackHttpHandlerFactory()
        {
            ...
            if (config == null)
            {
                throw new ConfigurationErrorsException(
                    "SStack: AppHost does not exist or has not been initialized. "
            ...
            //Apache+mod_mono treats path="SStack*" as path="*" so takes over root path, so we need to serve matching resources
            var hostedAtRootPath = config.SStackHandlerFactoryPath == null;
        ...
        public IHttpHandler GetHandler(HttpContext context, string requestType, string url, string pathTranslated)
        {
            ...
            var mode = EndpointHost.Config.SStackHandlerFactoryPath;
        ...
        private static IHttpHandler ReturnRequestInfo(HttpRequest httpReq)
        {
                ...
                || (EndpointHost.DebugMode && httpReq.PathInfo.EndsWith("__requestinfo")))
            {
                ...
                reqInfo.Host = EndpointHost.Config.DebugAspNetHostEnvironment + "_v" + Env.SStackVersion + "_" + EndpointHost.Config.ServiceName;
        ...
        private static IHttpHandler ReturnRequestInfo(IHttpRequest httpReq)
        {
                ...
                || (EndpointHost.DebugMode && httpReq.PathInfo.EndsWith("__requestinfo")))
            {
                ...
                reqInfo.Host = EndpointHost.Config.DebugHttpListenerHostEnvironment + "_v" + Env.SStackVersion + "_" + EndpointHost.Config.ServiceName;
                reqInfo.PathInfo = httpReq.PathInfo;
